...
# Log directory
os.makedirs('../train_log/ANN', exist_ok=True)

current_time = time.strftime('%Y-%m-%d %H-%M', time.localtime(time.time()))

log_dir = '../train_log/ANN/model_log-' + current_time + '.txt'

ann_dir = '../train_log/ANN/ann_' + current_time + '.pkl'
...
print('******** ANN ********\n', ann)
n_params_ann = function.show_num_params(ann)
print("\nTotal number of parameters: {0}\n".format(n_params_ann))

ann.cuda()

optimizer = torch.optim.Adam(ann.parameters(), lr=opt.lr)  # optimize all cnn parameters
