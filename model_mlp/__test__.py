...
file_list = list(os.listdir('../train_log/MLP'))
...
for file in file_list:
    ...
    if ('.pkl' in file) and ('mlp' in file):
...
mlp_dir = '../train_log/MLP/' + state_list[target]

mlp = torch.load(mlp_dir)
...
os.makedirs('../test_log/MLP', exist_ok=True)
log_dir = '../test_log/MLP/log_' + state_list[target].strip('.pkl').strip('mlp_') + '.xlsx'
predictions_dir = '../test_log/MLP/predictions_' + state_list[target].strip('.pkl').strip('mlp_') + '.xlsx'
