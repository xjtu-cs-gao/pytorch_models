...
# Log directory
os.makedirs('../train_log/RNN', exist_ok=True)

current_time = time.strftime('%Y-%m-%d %H-%M', time.localtime(time.time()))

log_dir = '../train_log/RNN/model_log-' + current_time + '.txt'

rnn_dir = '../train_log/RNN/rnn_' + current_time + '.pkl'
...
print('******** RNN ********\n', rnn)
n_params_rnn = function.show_num_params(rnn)
print("\nTotal number of parameters: {0}\n".format(n_params_rnn))

rnn.cuda()

optimizer = torch.optim.Adam(rnn.parameters(), lr=opt.lr)  # optimize all cnn parameters
